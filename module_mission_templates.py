...
###Equip Check###
equip_check=(
1,1,ti_once,[(get_player_agent_no,":pl"),(neq,":pl",-1)],
[
(assign,"$equip_set",0),
(get_player_agent_no,":pl"),
(try_begin),
(agent_has_item_equipped,":pl","itm_tb_head"),
(display_message,"@TB Set"),
(assign,"$equip_set",2),
(try_end),
(try_begin),
(this_or_next|agent_has_item_equipped,":pl","itm_iron_head"),
(agent_has_item_equipped,":pl","itm_iron_head_a"),
(display_message,"@IRON Set"),
(assign,"$equip_set",1),
(try_end),
])
###Equip Check###
...
###Skill###
skill_check=(
0,0.1,15,[(eq,"$equip_set",2),(key_clicked,key_j),(get_player_agent_no,":pl"),(neq,":pl",-1)],
[
...
(particle_system_burst, "psys_arrow_fire", pos28, 9),
(particle_system_burst, "psys_arrow_fire1", pos28, 9),
...
###TB Particle###
tb_particle=(
0.4,0,0,[(eq,"$equip_set",2),(this_or_next|game_key_is_down,gk_move_forward),(this_or_next|game_key_is_down,gk_move_backward),(this_or_next|game_key_is_down,gk_move_left),(game_key_is_down,gk_move_right),],
[
#(display_message,"@report:is_first_vision {$is_first_vision}"),
#(display_message,"@TB Move"),
(get_player_agent_no,":pl"),
(agent_get_position,pos26,":pl"),
(position_move_z,pos26,24,1),
(copy_position,pos27,pos26),
(store_random_in_range,":y1",-2,3),
(position_move_x,pos26,-35,0),
#(position_move_y,pos26,":y",0),
#(store_sub,":y3",1.18,":y1"),
(store_random_in_range,":y2",-2,3),
(position_move_x,pos27,35,0),
#(position_move_y,pos26,":y",0),

(try_begin),
(ge,":y1",0),
(particle_system_burst, "psys_tb_fire", pos26, 7),
(particle_system_burst, "psys_tb_fire1", pos26, 7),
(try_end),
(try_begin),
(ge,":y2",0),
(particle_system_burst, "psys_tb_fire1", pos27, 7),
(particle_system_burst, "psys_tb_fire", pos27, 7),
(try_end),
...
common_automatic = (0.05,0,0,[],[
	(try_for_agents,":attacker_agent"),
        ...
        (neg|eq,":agent_cur_weapon","itm_awp"),
...
(try_begin),
            ...
            (try_begin),
                ...
                (agent_set_attack_action, ":attacker_agent", 1),
            (try_end),
        (else_try),#player simulate attack
			...
			(try_begin),
                ...
                (try_begin),
                    (eq,":agent_cur_weapon","itm_test_rifle"),
                    (try_begin),
                        (gt,"$gatlin_i",0),
                        (val_sub,"$gatlin_i",1),
                    (try_end),
                    (try_begin),
                        (eq,"$gatlin_i",4),
                        (assign,"$g_automatic_ticker_time",3),
                    (else_try),
                        (eq,"$gatlin_i",0),
                        (assign,"$g_automatic_ticker_time",0),
                    (else_try),
                        (eq,"$gatlin_i",3),
                        (assign,"$g_automatic_ticker_time",2),
                    (else_try),
                        (assign,"$g_automatic_ticker_time",1),
                    (try_end),
                (else_try),
                    
                    (item_get_slot,"$g_automatic_ticker_time",":agent_cur_weapon",slot_item_shoot_freq),
                (try_end),
				#(store_div,"$g_automatic_ticker_time",":shoot_freq","$g_automatic_ticker_time"),
                ...
                (try_begin),
                    (lt,":cur_ammo_id",0),
                    (assign,":cur_ammo_id","itm_void_bullet"),#as default
                (try_end),
    ...
    ##adjust pos for horse_mode&crouch_mode##
				...
				(else_try),
                    #(display_message,"@on horse"),
					(position_move_z,pos24,253,1),
                (try_end),
                (try_begin),
       ...
       ##adjust pos for horse_mode&crouch_mode##
                ...
                (try_begin),
                (eq,":agent_cur_weapon","itm_test_rifle"),
                (val_max,":randomize_value",3),
                (try_end),
                (val_max,":randomize_value",1),
              ...
              #(try_end),
                ...
                (position_move_z,pos24,":pic_bit"),
				(play_sound_at_position, ":shoot_sound", pos24),
				(particle_system_burst, "psys_rifle_smoke", pos24, 5),
				(particle_system_burst, "psys_gun_fire", pos24, 5),

                (item_get_missile_speed,":missile_speed",":agent_cur_weapon"),
                (set_fixed_point_multiplier,1),
                (add_missile, ":attacker_agent", pos25, ":missile_speed", ":agent_cur_weapon", imod_plain, "itm_auto_bullet",imod_plain),
                #(add_missile, ":attacker_agent", pos25, 300, "itm_hunting_bow", imod_plain, "itm_test_arrow",imod_plain),

				#reduce ammo
                (try_begin),
                    (neq,":agent_cur_weapon","itm_test_rifle"),
                    #(val_sub,":cur_ammo",1),
		...
		(else_try),
            (try_begin),
                ...
                (assign,"$gatlin_i",5),
            (try_end),
            (item_get_slot,"$g_automatic_ticker_time",":agent_cur_weapon",slot_item_start_time),
            #(display_message,":start_time"),
            #(item_get_speed_rating,"$g_automatic_ticker_time",":agent_cur_weapon"),
            #(val_max,"$g_automatic_ticker_time",1),
			#(store_div,"$g_automatic_ticker_time",":start_time","$g_automatic_ticker_time"),
		(try_end),
	(try_end),
])
###AUTO SHOOT####

###CHECKING CAM TRIGGER#####
check_is_first=(
...
##          (agent_get_team, reg3, ":dead_agent_no"),          
      ...
      common_battle_order_panel,
      common_battle_order_panel_tick,
      check_is_first,common_automatic,tb_particle,equip_check,skill_check,
  ...
  (
    ...
    [
      ...
      (2, 0, ti_once, 
       ...
       common_battle_order_panel,
       common_battle_order_panel_tick,check_is_first,common_automatic,tb_particle,equip_check,skill_check,
...
("arena_duel_jack",mtf_arena_fight,-1,
"Dueling Jack",
[
  (0, mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_sword_medieval_a,itm_arena_tunic_blue]),
  (16, mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_sword_medieval_a,itm_arena_tunic_blue]),
 ...
 [common_inventory_not_available,
  (ti_tab_pressed,0,0,[(display_message,"@Jack:Trying to flee, Peasant?")],[]),
 (ti_before_mission_start, 0, 0, [], [(call_script, "script_change_banners_and_chest")]),
  ...
  (1,3,ti_once,[(main_hero_fallen)],[(display_message,"@Jack:Hah, you fucking loser!"),
	(quest_set_slot,"qst_mod_trouble",slot_quest_current_state,4),
	(str_store_string, s20, "@You losed the duel!"),
	(call_script,"script_fail_quest","qst_mod_trouble"),
   (finish_mission)]),
  (2,3,ti_once,[(all_enemies_defeated,1),(neg|main_hero_fallen,0)],
     [(display_message,"@Jack:No, I'm a loser...."),
      #(remove_troop_from_site,"trp_npc17","scn_town_mod_tavern"),
     (quest_set_slot,"qst_mod_trouble",slot_quest_current_state,3),
	(str_store_string, s20, "@You won the duel!"),
	(call_script,"script_succeed_quest","qst_mod_trouble"),
